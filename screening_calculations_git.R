...
##############################################################
## Pool size 32. If positive, test again in 8 pools of size 4. 
## Finally, test all 4 in the positive pools of size 4
##############################################################

sens32 <- 0.90 # Assumed sensitivity for pool size 32

# Expected number of postive pools of size 4 when the pool 
# of size 32 is positive.
ENPositivePools <- function(p)
  (dbinom(1,32,p)+(3/31+2*28/31)*dbinom(2,32,p)
   +(3*2/(31*30)+2*(3*28*3/(31*30))+3*(28*24/(31*30)))*dbinom(3,32,p)
   +4*dbinom(4,32,p)+5*dbinom(5,32,p)+6*dbinom(6,32,p)+7*dbinom(7,32,p)
   +8*pbinom(7,32,p, lower.tail=FALSE))/(1-dbinom(0,32,p))


# Number of persons that could be included with a 
# budget of Npcr tests and prevalence p. 
AntPersPool32 <- function(Npcr,p)
  32*Npcr/(1+poolprob(p,32,sens32)*(8+4*ENPositivePools(p)))
...
EAntDetectionsPool32 <- function(Npcr,p)
  poolprob(p,32,0.90)*Npcr/(1+poolprob(p,32,0.90)*(8+4*ENPositivePools(p)))*(32*p/(1-(1-p)^(32)))
